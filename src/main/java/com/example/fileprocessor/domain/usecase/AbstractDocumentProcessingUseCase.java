...
public abstract class AbstractDocumentProcessingUseCase {
     ...
     * Uses concatMap and collectList optimization for efficiency and reliability.
     *
    ...
    public Flux<FileUploadResponse> executePendingDocuments() {
        ...
        return Mono.deferContextual(ctx -> {
            ...
            LOGGER.info(() -> String.format("[TraceID: %s] Starting SEQUENTIAL execution for: %s", traceId,
                    implementationName()));
    ...
    /**
     * Fetches the document content from the source REST API and performs business
     * validations.
    ...
    private Mono<DocumentHistory> fetchAndValidate(DocumentHistory history) {
        return productRestGateway.getDocument(history.getProductId(), history.getBusinessDocumentId())
                ...
                .onErrorResume(ProcessingException.class, e -> {
                    ...
                    return Mono.error(
                            new ProcessingException(e.getErrorCode(), "Validation failed: " + e.getMessage(), e));
                });
    }

    /**
     * Finalizes the processing of a document by updating its state and saving the
     * audit history.
     *
     ...
     * @return a Mono of FileUploadResponse.
     */
    private Mono<FileUploadResponse> finalizeProcessing(Document doc, DocumentHistory history,
    ...
    /**
     * Determines the next state of the document based on the upload result and
     * retry logic.
     *
     ...
     * @return a Flux of DocumentHistory (one for each file inside the ZIP, or the
     *         original if not a ZIP).
     */
    ...
    /**
     * Abstract method to be implemented by specific providers (S3, SOAP, etc.) to
