...
public class R2dbcProductDocumentRepository implements ProductDocumentRepository {
    ...
    public Flux<ProductDocumentToProcess> findPendingDocuments() {
        String sql = """
            SELECT document_id, product_id, filename, origin, status, created_at, processed_at,
    ...
    public Mono<Void> save(ProductDocumentToProcess document) {
        String sql = """
            INSERT INTO product_documents_to_process (document_id, product_id, filename, origin, status,
        ...
        return databaseClient.sql(sql)
            ...
            .bind("$3", document.getFilename())
            .bind("$4", document.getOrigin())
            .bind("$5", document.getStatus())
            .bind("$6", document.getCreatedAt() != null ? document.getCreatedAt() : Instant.now())
            .bind("$7", document.getProcessedAt() != null ? document.getProcessedAt() : Instant.now())
            .bind("$8", document.getTraceId() != null ? document.getTraceId() : "")
            .bind("$9", document.getSoapCorrelationId() != null ? document.getSoapCorrelationId() : "")
            .bind("$10", document.getErrorCode() != null ? document.getErrorCode() : "")
            .fetch()
            .first()
            .then()
