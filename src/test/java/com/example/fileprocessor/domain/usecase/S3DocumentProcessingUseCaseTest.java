...
class S3DocumentProcessingUseCaseTest {
    ...
    void executePendingDocumentsWithS3Success() {
        ...
        when(s3Gateway.send(any(ProductUploadRequest.class))).thenReturn(Mono.just(FileUploadResponse.builder()
            .success(true)
    ...
    void executePendingDocumentsWithS3Failure() {
        ...
        when(s3Gateway.send(any(ProductUploadRequest.class))).thenReturn(Mono.error(new RuntimeException("S3 connection error")));
